...
import {
  answeredNumbers, createSession, goTo, questionSlots, remainingSeconds, setAnswer, submit, tick, toggleFlag,
...
const clockKey = (id: string) => `vocabbrain:testClock:${id}`;
...
export function TestRunner({ test, skill }: { test: PracticeTest; skill: ObjectiveSkill }) {
  ...
  // updates come from this component and must not reload over live answers.
  const [initialId] = useState(() => params.get('session'));

  // Load: a specific attempt (?session=) or the unfinished one for this test.
  useEffect(() => {
    let cancelled = false;
    const id = initialId;
  ...
  if (!session) {
    ...
    return <TestIntro test={test} skill={skill} section={section} resumable={resumable} onStart={start} />;
  }
